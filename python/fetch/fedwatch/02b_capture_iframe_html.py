...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        iframe_locator = host_page.locator("iframe.cmeIframe").first
        try:
            iframe_locator.wait_for(timeout=60000)
        except Exception:
            iframe_locator = host_page.locator("iframe").first
            iframe_locator.wait_for(timeout=60000)
        ...
        frame = host_page.frame(url=re.compile(r"quikstrike", re.IGNORECASE))
        if frame is None:
            frame = host_page.frame(url=iframe_url)
        if frame is None:
        ...
        final_url = frame.url
        page_title = host_page.title()
        try:
            body_text = frame.inner_text("body")
        except Exception:
            body_text = ""

        iframe_locator.screenshot(path=str(out_png))
        try:
