...
SL/TP rule-based (structure-based SL + TP in R units)
...
Works on H4 chart; evaluate only at H4 candle close.
Note on candle indexing (MQL5 series):
- Use LAST CLOSED H4 candle only for entry signals.
- Use index 1 for signal checks (Close[1], High[1], Low[1]).
- Index 0 (current forming candle) must NOT be used for entry decisions.

One open position at a time (for EURUSD, magic number). If any EURUSD position by this magic is open, do not open new ones; only manage TP1/BE/TP2.
Terminology clarification:
- Use "1 position" (not "1 ออเดอร์") to avoid confusion.
- Partial close creates multiple deals/log rows, but the position is still a single open position until fully closed.
...
Setup A: Break & Retest (use last closed candle only)
...
price returns near level within tolerance using the last closed candle
(e.g., abs(Low[1]-level) < 0.2\*ATR for LONG)

Confirmation:

latest closed H4 candle closes in direction (bullish for LONG / bearish for SHORT)

Setup B: Pullback to Zone
...
price enters zone AND latest closed candle closes back in bias direction

SL rule (rule-based, structure-based)
...
TP + scale-out (rule-based, in R units)

Let R = abs(entry - SL) in price. TP1/TP2 are defined in R units derived from entry and SL.

TP1 = entry + R (LONG) or entry - R (SHORT)

TP2 = entry + 2R or entry - 2R
On TP1 hit:
...
log a row for the partial deal with lots = closed_lots for TP1 and pnl_r = realized_pnl_money / R_money
Final close:
